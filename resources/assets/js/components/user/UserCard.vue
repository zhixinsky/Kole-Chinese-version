  ...
  >
    ...
    <div class="space-x-2">
      <template v-if="user.is_prospect">
        <Btn class="btn-revoke" danger small @click="revokeInvite">撤销</Btn>
      </template>
      <template v-else>
        <Btn v-if="!user.is_prospect" highlight small @click="edit">
          {{ isCurrentUser ? '您的个人资料' : '编辑' }}
        </Btn>
        <Btn v-if="!isCurrentUser" danger small @click="destroy">删除</Btn>
      </template>
    </div>
  </article>
</template>
...
const destroy = async () => {
  if (!await showConfirmDialog(`删除 ${user.value.name}?`)) {
    return
  }

  await userStore.destroy(user.value)
  toastSuccess(`"${user.value.name}" 用户已删除。`)
}

const revokeInvite = async () => {
  if (!await showConfirmDialog(`撤销邀请 ${user.value.email}?`)) {
    return
  }
