<template>
  ...
  >
    ...
    <span
      ...
      :title="`按 ${contentType === 'episodes' ? 'podcast' : (contentType === 'songs' ? '专辑' : 'album/podcast')}`"
      class="album"
    ...
    >
      <template v-if="contentType === 'episodes'">播客</template>
      <template v-else-if="contentType === 'songs'">专辑</template>
      <template v-else>专辑 <span class="opacity-50">/</span> 播客</template>
    ...
    <template v-if="config.collaborative">
      <span class="collaborator">用户</span>
      <span class="added-at">已加入</span>
    </template>
    <span
