<template>
  <article>
    <button ref="button" class="w-full focus:text-k-highlight" title="种类" @click.stop="trigger">
...
const menuItems = computed(() => {
  const title: MenuItem = { column: 'title', label: '歌名', field: 'title', visibilityToggleable: false }
  const artist: MenuItem = { label: '艺术家', field: 'artist_name', visibilityToggleable: false }
  const author: MenuItem = { label: '作者', field: 'podcast_author', visibilityToggleable: false }
  ...
  const album: MenuItem = { column: 'album', label: '专辑', field: 'album_name', visibilityToggleable: true }
  const track: MenuItem = { column: 'track', label: '光盘', field: 'track', visibilityToggleable: true }
  const time: MenuItem = { column: 'duration', label: '时长', field: 'length', visibilityToggleable: true }
