<template>
  ...
  >
    ...
    <template #meta>
      <a :title="`随机播放此艺术家的所有歌曲 ${artist.name}`" role="button" @click.prevent="shuffle">
        随机
      </a>
      <a v-if="allowDownload" :title="`下载此艺术家的所有歌曲 ${artist.name}`" role="button" @click.prevent="download">
        下载
      </a>
    </template>
  </BaseCard>
